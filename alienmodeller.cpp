...
{
    QPolygon *tempJnt2JntPoly = alien->alienSkeleton.Jnt2JntRelations_TypeGuided.value(BP_ID);

    BodyPart *tempThisBP_Ptr = getBodyPartFromID(alien, BP_ID); // TODO(ismail): check if a nullptr is being returned or not

    qDebug() << *tempJnt2JntPoly;
    qDebug() << tempThisBP_Ptr->BP_Type;

    // pick up a root joint (at random) (** NOT USING RANDOM AT THE MOMENT **)
    // always select the first joint as the root joint
    int randRootJnt = tempJnt2JntPoly->at(0).x();

    tempThisBP_Ptr->rootJntID = randRootJnt; // store the root joints' ID in this BP object. THIS IS THE FIRST place where the root joint is set
    alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided[randRootJnt].second = QVector3D(newBP_startingX_Loc, 0, 0);
    // TODO(Ismail): Check if this is causing the joints to not appear in the right position
//    tempThisBP_Ptr->BP_TranslateMatrix.translate(alienList.last()->alienSkeleton.tempVectorLocationCoupleForEachJoint_TypeGuided[randRootJnt].second);
    ...
    QPolygonF temp_BP_JAT_DNA_Poly = tempCopy_BP_JAT_rootJoint_DNA.value(tempBP_Type_ID);
    // Store it in the BP_to_JAT_rootJoint_Map_DNA for this BP. This will be used later for cloning purposes
    QPolygonF temp_BP_JAT_DNA_Poly_2; temp_BP_JAT_DNA_Poly_2.resize(0);
    temp_BP_JAT_DNA_Poly_2 << temp_BP_JAT_DNA_Poly.at(0) << temp_BP_JAT_DNA_Poly.at(1);
    if (!alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.BP_to_JAT_rootJoint_Map_DNA.contains(BP_ID))
        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.BP_to_JAT_rootJoint_Map_DNA.insert(BP_ID, temp_BP_JAT_DNA_Poly_2);

    qDebug() << "reached 3";

//        randomEnergyMultipleFromDNA /= 2;
    // update the energy and angle trends of the root joint
    alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided[randRootJnt][0].setX(temp_BP_JAT_DNA_Poly.at(0).x());
    alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided[randRootJnt][1].setX(temp_BP_JAT_DNA_Poly.at(1).x());

    float randomEnergyMultipleFromDNA = temp_BP_JAT_DNA_Poly.at(0).y();
    float randomAngleFromDNA = temp_BP_JAT_DNA_Poly.at(1).y();

    temp_BP_JAT_DNA_Poly.removeFirst();
    temp_BP_JAT_DNA_Poly.removeFirst();

    if (tempCopy_BP_JAT_rootJoint_DNA.remove(tempBP_Type_ID) == 1)
        tempCopy_BP_JAT_rootJoint_DNA.insert(tempBP_Type_ID, temp_BP_JAT_DNA_Poly);

    QPolygonF tempRootJntPolygon = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.value(randRootJnt);
    for (int i=0; i<tempJnt2JntPoly->length(); i++)
    {
        ...
        QPolygonF tempPolygon = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.value(parentJnt);
        for (int j=i; j<tempJnt2JntPoly->length(); j++)
        {
            ...
            if (tempJnt2JntPoly->at(j).x() == parentJnt  &&  tempChildJntPositionCheckList.value(childJnt) == 0) // position not finalized yet..so go ahead
            {
                ...
                QPolygonF tempPolygon2 = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.value(childJnt);
                //
                // ********* ENERGY -- first get the energy sorted *********** // TODO(ismail): can use switch statement here
                //
                int tempRandInt = 0;
                float tempRandFloat = 0.0;
                if (tempRootJntPolygon[0].x() == 4.0) // if the energy trend at the root joint is irregular/varying
                {
                    if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_EnergyTrend.contains(childJnt))
                        tempRandInt = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_EnergyTrend.value(childJnt);
                    else
                    {
                        tempRandInt = GeometryAlgorithms::getRandomInteger(-100, 100);
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_EnergyTrend.insert(childJnt,tempRandInt);
                    }
                    if (tempRandInt < 0)    // negative means decending
                        tempPolygon[0].setX(1.0);
                    else                    // positive means ascending
...
//                        tempPolygon2[0].setY(GeometryAlgorithms::getRandomFloat(tempPolygon[0].y(), GeometryAlgorithms::getRandomInteger(50,150) + tempPolygon[0].y()));
                    if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.contains(childJnt))
                        tempRandFloat = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.value(childJnt);
                    else
                    {
                        tempRandFloat = GeometryAlgorithms::getRandomFloat( tempPolygon[0].y(), randomEnergyMultipleFromDNA * tempPolygon[0].y() );
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.insert(childJnt,tempRandFloat);
                    }
                    tempPolygon2[0].setY(tempRandFloat);
...
//                    qDebug() << "ENERGY ASCENDING";
                ...
                else if (tempPolygon[0].x() == 1.0) // if the energy trend of its parent is DESCENDING
                {
                    ...
                    {
                        tempRandFloat = GeometryAlgorithms::getRandomFloat( 0.8*tempPolygon[0].y(),tempPolygon[0].y() );
//                        tempRandFloat = GeometryAlgorithms::getRandomFloat( (1/randomEnergyMultipleFromDNA)*tempPolygon[0].y(),2*tempPolygon[0].y() );
                        // Another option is below
//                        tempRandFloat = GeometryAlgorithms::getRandomFloat( tempPolygon[0].y()/2,tempPolygon[0].y() );
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.insert(childJnt,tempRandFloat);
                    }
                    tempPolygon2[0].setY(tempRandFloat);
...
//                    qDebug() << "ENERGY WAVY UP";
                ...
                else if (tempPolygon[0].x() == 3.0) // if the energy trend of its parent is WAVY DOWN
                {
                    ...
                    {
                        tempRandFloat = GeometryAlgorithms::getRandomFloat( 0.8*tempPolygon[0].y(),tempPolygon[0].y() );
//                        tempRandFloat = GeometryAlgorithms::getRandomFloat( (1/randomEnergyMultipleFromDNA)*tempPolygon[0].y(),tempPolygon[0].y() );
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.insert(childJnt,tempRandFloat);
                    }
                    tempPolygon2[0].setY(tempRandFloat);
...
//                    qDebug() << "ENERGY WAVY DOWN";
                ...
                else if (tempPolygon[0].x() == 5.0) // if the energy trend of its parent is CONSTANT
                {
                    if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.contains(childJnt))
                        tempRandFloat = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.value(childJnt);
                    else
                    {
                        tempRandFloat = tempPolygon[0].y();
//                        tempRandFloat = GeometryAlgorithms::getRandomFloat( (1/randomEnergyMultipleFromDNA)*tempPolygon[0].y(),tempPolygon[0].y() );
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randFloatForBookKeeping_FinalEnergy.insert(childJnt,tempRandFloat);
                    }
                    tempPolygon2[0].setY(tempRandFloat);
                    tempPolygon2[0].setX(5);
//                    qDebug() << "ENERGY WAVY DOWN";
                }

                //
                // ********* ANGLE -- Next get the angle sorted ***********
                //
                if (tempRootJntPolygon[1].x() == 4.0) // if the angle trend at the root joint is irregular/varying
                {
//                    qDebug() << "ANGLE TREND IS IRREGULAR/VARYING";
                    if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_AngleTrend.contains(childJnt))
                        tempRandInt = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_AngleTrend.value(childJnt);
                    else
                    {
                        tempRandInt = GeometryAlgorithms::getRandomInteger(-100, 100);
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_AngleTrend.insert(childJnt,tempRandInt);
                    }
                    if (tempRandInt < 0)
...
//                    qDebug() << "ANGLE WAVY UP";
                ...
                // TODO(ismail): perhaps get rid of remove() as the updated Polygon replaces the old one in insert()
                if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.remove(parentJnt) == 1)
                    alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.insert(parentJnt, tempPolygon);
                if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.remove(childJnt) == 1)
                    alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.VectorEnergyAngleTrends_TypeGuided.insert(childJnt, tempPolygon2);
                else
                {
                    qDebug() << "doesn't work";
                    exit(1);
...
//                    qDebug() << "isThisFirstChild reached";
                    // TODO(ismail): Inspect this. Is this working fine?
                    if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_quaterEulerAngleSelectorForFirstChild.contains(childJnt))
                        tempRandInt = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_quaterEulerAngleSelectorForFirstChild.value(childJnt);
                    else
                    {
                        tempRandInt = GeometryAlgorithms::getRandomInteger(-100, 100);
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randIntForBookKeeping_quaterEulerAngleSelectorForFirstChild.insert(childJnt,tempRandInt);
                    }

                    if (tempRandInt < 0)
                        vecOrientQuat = QQuaternion::rotationTo(QVector3D(0.0,0.0,1.0), alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided.value(parentJnt).first)
                ...
                else // for further branchings and splitings. Not being used at the moment
                {
                    // TODO(ismail): Inspect this. Is this working fine?
//                    vecOrientQuat = QQuaternion::fromEulerAngles((jntsArrangeTrends.last().VectorEnergyAngleTrends[tempJnt2].at(2)), 0.0, 0.0)
//                                    * QQuaternion::rotationTo(QVector3D(0.0,0.0,1.0), tempVectorLocationCoupleForEachJoint[tempJnt].at(0));
                    QVector3D tempVec3D = QVector3D(0,0,0);
                    if (alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randQVector3DForBookKeeping_quaterEulerAngleSelectorForNotFirstChild.contains(childJnt))
                        tempVec3D = alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randQVector3DForBookKeeping_quaterEulerAngleSelectorForNotFirstChild.value(childJnt);
                    else
                    {
                        tempVec3D.setX(GeometryAlgorithms::getRandomFloat(5.0,60.0));
                        tempVec3D.setY(GeometryAlgorithms::getRandomFloat(5.0,60.0));
                        tempVec3D.setZ(0.0);
                        // store it so it can be used by the clone in future
                        alien->alienDNA->skeletonDNA->skeletal_JATs_DNA.randQVector3DForBookKeeping_quaterEulerAngleSelectorForNotFirstChild.insert(childJnt,tempVec3D);
                    }

                    vecOrientQuat = QQuaternion::rotationTo(QVector3D(0.0,0.0,1.0), alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided.value(parentJnt).first)
                                    * QQuaternion::fromEulerAngles(tempVec3D.x(),tempVec3D.y(),tempVec3D.z());

                    isJntAChild[(alien->alienJoints[i]->BP_ID)].setY(1);
//                    vecOrientQuat = QQuaternion::fromEulerAngles((jntsArrangeTrends.last().VectorEnergyAngleTrends[tempJnt2].at(2)), 0.0, 0.0);
                }
...
//                QVector3D tempOrientVector = (vecOrientQuat.rotatedVector(tempVectorLocationCoupleForEachJoint[tempJnt].at(0))).normalized();
                ...
                // now compute the joint's location using the energy computed in the code above
                QVector3D newLoc =  alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided.value(parentJnt).second + (tempPolygon2[0].y() * tempOrientVector);

//                    qDebug() << "joint " << tempJnt2 << " has location : " << newLoc;

                alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided[childJnt].first = tempOrientVector;
                alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided[childJnt].second = newLoc;
                ...
                // This is just to store the parent and child joint's locations to be rendered as opengl lines
                QPair<QVector3D,QVector3D> vec3DPair;
                vec3DPair.first = alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided.value(parentJnt).second;
                vec3DPair.second = alien->alienSkeleton.vectorLocationCoupleForEachJoint_TypeGuided.value(parentJnt).second + (tempPolygon2[0].y() * tempOrientVector);
                alien->tempVectorsListForRendering.append(vec3DPair);
            }
            isThisChildFirst=false;
        }
    ...
    qDebug() << "JAT_AND_VecLocCplForEachJnt built";
}
